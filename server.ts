...
async function startServer() {
  ...
  // Update a record by ID (using POST due to proxy restrictions on PUT/DELETE)
  ...
  // Delete a record by ID (using POST to a /delete endpoint)
  ...
  // API Catch-all: Ensure that any missing /api/* route returns 404 JSON instead of Vite HTML
  app.all('/api/*', (req, res) => {
    res.status(404).json({ 
